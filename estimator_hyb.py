...
def drop_and_solve_hybOpt(f, n, alpha, q, bound, secret_distribution=True, success_probability=0.99,
    ...
    print("k", k, "l1", l1, "l2", l2, "h1", h1, "h2", h2)
    return best
...
def MITM_estimator(n, alpha, q, start_bound, step_size, bound_bound):
    bound = float(start_bound)
    dualddd = partial(drop_and_solve_hybOpt, dual_scale_hyb)
    print("== constants == ")
    print("n = " + str(n) + ", q = " + str(q))
    print("")
    cost_best_bound = 0
    while step_size >= 1/4:
        print("do with bound " + str(bound))
        res = dualddd(n, alpha, q, bound, secret_distribution=((-1, 1), 64))
        rop_key = list(res)[0]
        ...
        if cost_best_bound is 0 or rop < cost_best_bound:
            cost_best_bound = rop
            bound += step_size
            while bound >= bound_bound and step_size >= 1/4:
