...
def drop_and_solve_hyb(f, n, alpha, q, bound, secret_distribution, **kwds):
    '''
    * Input 'bound' means,
...
def hybrid_dual(n, alpha, q, secret_distribution, m, start_bound = 10, Max_bound = 13, step_size = 1, reduction_cost_model = reduction_default_cost):
    bound = float(start_bound)
    mitm_hyb = partial(drop_and_solve_hyb, dual_scale_hyb)
    best = None
    bound_best = bound
    Level = 0
    ...
    while Level <= 2:
        ...
        res = mitm_hyb(n, alpha, q, bound, secret_distribution=secret_distribution, reduction_cost_model=reduction_cost_model)
...
def estimate_lwe(n, alpha=None, q=None, secret_distribution=True, m=oo, # noqa
    ...
    if "hyb-dual" not in skip:
        if SDis.is_ternary(secret_distribution):
            algorithms["hyb-dual"] = partial(hybrid_dual, reduction_cost_model=reduction_cost_model)
        else:
            raise ValueError("Currently Hybrid-dual supports only ternary secret case.")
